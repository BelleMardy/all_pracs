...
def main():
    amount_of_quick_picks = int(input("How many quick picks would you like?: >>> "))
    while amount_of_quick_picks < 0:
        amount_of_quick_picks = int(input("You need to enter a number > 0: >>> "))
    for i in range(amount_of_quick_picks):
        quick_picks = []
        for choice in range(NUMBERS_PER_LINE):
